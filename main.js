...
function createBase() { // Baseシートの自動作成
  ...
  for (let j = 0; j < place.length; j++) { // 実施場所毎のループ（1つの表）
    // ここでセルに色を付けたい
    baseSheet.getRange(totalStartRowNum + tableRowCount, 3, groupCount[j], statisticOption.length).setBackground('aqua');
    ...
    baseSheet.getRange(totalStartRowNum + tableRowCount, 3 + statisticOption.length, 1, 1).setFormulaR1C1('=SUM(RC[' + (-statisticOption.length) + '],RC[-1])');
    placeStatisticLines.push(tableRowCount++);
  }

  baseSheet.getRange(totalStartRowNum + tableRowCount, 2, 1, 1).setValue('合計').setFontColor('red');
  for (let j = 0; j < placeStatisticLines.length; j++) {
    placeStaticFormula += 'R' + placeStatisticLines[j] + 'C';
    if (j + 1 !== placeStatisticLines.length) placeStaticFormula += '+';
  }
  ...
  totalStartRowNum += tableRowCount + 1;
}
//}